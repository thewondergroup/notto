...
- **Going live (custom domain)** — `CNAME` contains `www.nottopastabars.com`; keep it in the repo root or GitHub Pages forgets the domain. `robots.txt` and `sitemap.xml` point at that domain, as do the canonical tags on every page. If the domain changes, update all four.
